...
# Load Image
camera_name = "fisheye_model"
img_file_name = "1"
fisheye_img = cv2.imread('./sample/'+camera_name+ "_" +img_file_name+".jpg")
M, N = int(fisheye_img.shape[1]), int(fisheye_img.shape[0])
print("image size: "+str(M)+" x "+str(N))

# intrinsic parameters
a = np.loadtxt('./parameters/'+camera_name+'_intrinsic.csv', delimiter=",", dtype=np.float32 ) 
u0, v0 = a[6], a[5]
...
def equirec2cylin(u_, v_):
    ...
    # np.savetxt('./parameters/'+camera_name+"_lambda.csv",l, delimiter=',')
    # np.savetxt('./parameters/'+camera_name+"_phi.csv",p, delimiter=',')
...
def cylin2polar(l, p):
    ...
    # np.savetxt('./parameters/'+camera_name+"_theta.csv",theta, delimiter=',')
    # np.savetxt('./parameters/'+camera_name+"_alpha.csv",alpha, delimiter=',')
    return theta, alpha
...
def polar2fisheye(alpha, rho):
    ...
    np.savetxt('./parameters/'+camera_name+"_u.csv",u, delimiter=',')
    np.savetxt('./parameters/'+camera_name+"_v.csv",v, delimiter=',')
    print("Complete to compute u and v")
...
def get_rho(theta):
    num = 0
    ans = np.zeros((N,M))
    if not os.path.isfile('./parameters/'+ camera_name + '_rho.csv') :
        ...
        for row in theta:
            ...
            for col in row:
                ...
                i  = i + 1
            num = num + 1
        pd.DataFrame(ans).to_csv("./parameters/" + camera_name + "_rho.csv", header=False, index=False)
        print("Complete to compute rho file")
    else:
        print("rho file exists!")
        ans = pd.read_csv('./parameters/'+ camera_name +'_rho.csv', delimiter=",",header=None, dtype=np.float32).values
...
if __name__ == '__main__':
    ...
    # u = pd.read_csv("./parameters/"+camera_name+"_u_param.csv", delimiter=",",header=None, dtype=np.float32).values
    # print("Complete to load u_param")
    # v = pd.read_csv("./parameters/"+camera_name+"_v_param.csv", delimiter=",",header=None, dtype=np.float32).values
    # print("Complete to load v_param")

    img = cv2.remap(fisheye_img, u, v, cv2.INTER_LINEAR)
    cv2.imwrite('./output/'+camera_name+ "_" +img_file_name+'_result.jpg', img)
    print("Finished!")
